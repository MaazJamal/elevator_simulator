...
passengers_take_in(ell_arr,1,floor_arr,1);
}
...
ell_arr[1] = dummy_2;
struct passenger** floor_arr = malloc(sizeof(struct passenger*) * 10);
passengers_take_in(ell_arr,1,floor_arr,10);
}
...
passengers_take_in(ell_arr,1,floor_arr,5);
}
...
int added = passengers_take_in(ell_arr,1,floor_arr,5);
int okay = 0;
for (int i = 0; i<10;i++){
