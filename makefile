...
SRC = src/globals.c src/add_passenger_floor.c src/add_request_queue.c src/count_passenger.c src/create_elevator_array.c src/create_floor_array.c\
 ...
 src/static_simulation.c src/dynamic_simulation.c src/cli_update.c src/input_after_stop.c \
 src/input_static_dynamic.c src/emergency_stop_handling.c src/read_user_input_validate.c
...
debugtests: test/test_main.c $(SRC) $(TEST)
	$(CC) -g $(TESTCFLAGS) $? -o $@
