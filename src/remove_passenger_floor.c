...
#include "stdio.h"
/**
 * @file remove_passenger_floor.c
 *
 * @brief Removes the passenger details from floor specified.
 *
 ...
 * @param[in] The function takes passenger structure and current floor.
 * @return Returns 1 on successful deletion of passenger details from specified floor.
 */
...
int remove_passenger_floor(struct passenger *floordetails[], int cur_floor)
{
	...
	printf("Passenger %d from floor %d removed. No of passengers removed: %d\n", temp->id, cur_floor, ++no_of_passengers_removed);
	free(temp);
	return 1;
}
